...
angular
  ...
  .controller(
    "BoardCtrl",
    function ($scope, $timeout, $modal, socket, currencyFilter) {
      ...
      socket.on("fj:reveal", function (playerId, playerInfo) {
        console.log("board got fj:reveal" + playerId + playerInfo);
        if (modalInstance) {
          modalInstance.close();
        }
        modalInstance = $modal.open({
          templateUrl: "partials/finalreveal",
          controller: "FinalRevealCtrl",
          backdrop: "static",
          size: "lg",
          openedClass: "board-modal-open",
          resolve: {
